...
def BoundaryHandling(x,lb,ub,BoundHandling,lb_tot_eros=None,ub_tot_eros=None): 
    ...
    if ((BoundHandling=='Reflect') | (BoundHandling=='CRN')):
...
def Metrop(MCMCPar,xnew,log_p_xnew,xold,log_p_xold,alfa_s):
    ...
    if (MCMCPar.Prior[0:9]=='Prior_CRN'):
        ...
        for zz in xrange(0,MCMCPar.seq):
            ...
            if (MCMCPar.Prior[10]=='1'): # Gaussian prior(s) for the first MCMCPar.ngp variables
                log_prior_new[zz,0] += -0.5 * reduce(np.dot,[(xnew[zz,:MCMCPar.ngp]-MCMCPar.pmu),(MCMCPar.invC),(xnew[zz,:MCMCPar.ngp]-MCMCPar.pmu).T])
                
            # Bounded uniform prior on the total erosion (erosion rate x age)
            if ((xnew[zz,0]*xnew[zz,1]<(MCMCPar.lb_tot_eros*1e6)) | (xnew[zz,0]*xnew[zz,1]>(MCMCPar.ub_tot_eros*1e6)) ):
                log_prior_new[zz,0]=-1e300
                
            if (MCMCPar.Prior[10]=='1'): # Gaussian prior(s) for the first MCMCPar.ngp variables
                log_prior_old[zz,0] += -0.5 * reduce(np.dot,[(xold[zz,:MCMCPar.ngp]-MCMCPar.pmu),(MCMCPar.invC),(xold[zz,:MCMCPar.ngp]-MCMCPar.pmu).T])
            
            if ((xold[zz,0]*xold[zz,1]<(MCMCPar.lb_tot_eros*1e6)) | (xold[zz,0]*xold[zz,1]>(MCMCPar.ub_tot_eros*1e6)) ):
                log_prior_old[zz,0]=-1e300
...
def RunFoward(X,MCMCPar,Measurement,ModelName,Extra,DNN=None):
    ...
    elif ModelName=='forward_model_crn':
        extra_par=[]
